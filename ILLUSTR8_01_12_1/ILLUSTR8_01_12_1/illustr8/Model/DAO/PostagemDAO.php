...
class PostagemDAO {
    ...
    public function alterarPostagem( PostagemDTO $postagemDTO ) {
        try {
            $con = Conexao::getInstance();
            $sql = "UPDATE POSTAGENS SET TITULO=?, TIPO=?, DESCRICAO=?, PRECO=? WHERE ID=? ";
            // $sql .= " VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)";
            $stmt = $con->prepare( $sql );
            $stmt->bindValue( 1, $postagemDTO->getTitulo() );
            $stmt->bindValue( 2, $postagemDTO->getTipo() );
            $stmt->bindValue( 3, $postagemDTO->getDescricao() );
            $stmt->bindValue( 4, $postagemDTO->getPreco() );
            $stmt->bindValue( 5, $postagemDTO->getIdPostagem() );
